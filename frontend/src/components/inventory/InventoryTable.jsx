export default function InventoryTable({ items = [], loading, error, onEdit, onReceive, onUse, onAdjust }) {
            ...
            <tr key={item.id} className="hover:bg-blue-50/60">
              ...
              <td className="px-4 py-3">
                <div className="flex flex-wrap justify-end gap-2">
                  <button className="font-semibold text-blue-700 hover:underline" onClick={() => onReceive?.(item)}>Receive</button>
                  <button className="font-semibold text-emerald-700 hover:underline" onClick={() => onUse?.(item)}>Use</button>
                  <button className="font-semibold text-amber-700 hover:underline" onClick={() => onAdjust?.(item)}>Adjust</button>
                </div>
              </td>
            </tr>
          ))}
